...
USAGE:
    ...
    # Population as a raster zonal SUM (not a mean or an area percentage
    # -- a population-count raster's total for a zone is the sum of every
    # pixel inside it; see join_raster_sum()'s docstring for why summing
    # is the only correct aggregation here):
    python3 -m data_pipeline.static_datasets.spatial_join \\
        --mode raster_sum \\
        --raster-path ind_ppp_2020_1km_Aggregated.tif \\
        --dataset ind_ppp_2020_1km_Aggregated.tif \\
        --out population.csv
...
--mode raster_sum additionally requires rasterstats:
...
def join_raster_sum(zones: "gpd.GeoDataFrame", raster_path: str) -> dict:
    """Sum every raster pixel's value inside each zone's boundary. Use
    this for count-per-pixel rasters (WorldPop population) where the
    total for a zone is a SUM, not a mean or an area percentage --
    getting this wrong (e.g. averaging density instead of summing count)
    silently produces nonsense totals. Zones with no raster coverage at
    all come back as None, not 0 -- a genuinely empty area and a missing
    raster tile are different things and must not be conflated."""
    ...
    stats = zonal_stats(
        ...
        raster_path,
        stats=["sum"],
        nodata=-99999,  # WorldPop's standard nodata sentinel; adjust if your raster uses a different one
    )
    result = {
        zone_id: (s["sum"] if s["sum"] is not None else None)
        for zone_id, s in zip(zones["zone_id"], stats)
    }
    return result
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--dataset", required=True, help="Downloaded source dataset (shapefile/GeoJSON, or the raster path again for --mode raster_sum)")
    parser.add_argument("--out", required=True, help="Output CSV path, ready for load_*.py")
    parser.add_argument("--mode", choices=["value", "area_pct", "raster_sum"], default="value")
    ...
    parser.add_argument("--raster-path", help="Required for --mode raster_sum: path to the GeoTIFF raster")
    ...
    elif args.mode == "raster_sum":
        if not args.raster_path:
            parser.error("--raster-path is required for --mode raster_sum")
        try:
            import rasterstats  # noqa: F401
        except ImportError:
            print("rasterstats is required for --mode raster_sum: pip install rasterstats --break-system-packages", file=sys.stderr)
            return 1
        result = join_raster_sum(zones, args.raster_path)
    else:
