...
# Ensure hazard_platform root is in sys.path so submodules resolve cleanly
_HP_ROOT = str(Path(__file__).resolve().parent.parent)
...
from evacuation.capacity import compute_zone_capacities, map_safe_shelters
from evacuation.planner import (
    PlannerCache,
    make_hazard_reader,
    plan_all_zones,
    plan_zone,
...
_hazard_reader = make_hazard_reader(store)
...
def _bbox_from_point(lon: float, lat: float, radius_km: float = 5.0):
    if BBox is None:
        raise HTTPException(
            status_code=503,
            detail="gis_fetcher package not installed alongside this API",
        )
...
def analyze_point(lat: float, lon: float, radius_km: float = 5.0):
    ...
    # Map safe shelters: public view gets location + type, no capacity numbers (FINAL doc §11)
    shelters = shelters_mod.get_shelters(zone.zone_id)
    caps = compute_zone_capacities(shelters, classification.color)
    safe_shelters = map_safe_shelters(caps)
...
@app.get("/api/authority/plan/{zone_id}")
def get_authority_plan(zone_id: str):
    """Authority evacuation plan for a specific zone.
    Returns complete breakdown: population demand, shelter effective capacities,
    capacity gap (unserved), and full shelter details.
    Cached by (zone_id, reading_timestamp).
    """
    try:
        plan = plan_zone(zone_id, _hazard_reader, cache=_planner_cache)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed to compute evacuation plan: {exc}") from exc

    zone = get_zone(zone_id)
    plan_dict = plan.to_dict()
    if zone:
        plan_dict["zone_name"] = zone.name
        parts = zone.zone_id.split("-")
        plan_dict["state"] = parts[1].title() if len(parts) > 2 else ""
        plan_dict["center"] = {"lat": zone.center[1], "lon": zone.center[0]}
        plan_dict["bbox"] = [zone.min_lon, zone.min_lat, zone.max_lon, zone.max_lat]
    return plan_dict


@app.get("/api/authority/plans")
def get_authority_plans():
    """Authority overview across all known zones.
    Computes/returns plans for all ingested zones and reports overall summary statistics.
    """
    all_zones = list_zones()
    zone_ids = [z.zone_id for z in all_zones]
    batch_result = plan_all_zones(zone_ids, _hazard_reader, cache=_planner_cache)

    plans_data = {}
    summary_stats = {
        "total_zones": len(all_zones),
        "ingested_zones": len(batch_result["plans"]),
        "unseeded_or_untested_zones": len(batch_result["errors"]),
        "total_population": 0.0,
        "total_demand": 0,
        "total_assignable_capacity": 0.0,
        "total_unserved": 0.0,
        "red_zones_count": 0,
        "yellow_zones_count": 0,
        "green_zones_count": 0,
        "trapped_zones_count": 0,
        "total_shelters": 0,
        "total_safe_shelters": 0,
        "total_registered_shelters": len(shelters_mod.get_all_shelters()),
    }


    zone_map = {z.zone_id: z for z in all_zones}

    for zid, plan in batch_result["plans"].items():
        pd = plan.to_dict()
        z = zone_map.get(zid)
        if z:
            pd["zone_name"] = z.name
            parts = z.zone_id.split("-")
            pd["state"] = parts[1].title() if len(parts) > 2 else ""
            pd["center"] = {"lat": z.center[1], "lon": z.center[0]}
            pd["bbox"] = [z.min_lon, z.min_lat, z.max_lon, z.max_lat]
        plans_data[zid] = pd

        pop = pd.get("demand", {}).get("population") or 0.0
        dem = pd.get("demand", {}).get("demand") or 0
        assignable = pd.get("capacity_summary", {}).get("total_assignable") or 0.0
        unserved = pd.get("capacity_gap", {}).get("unserved") or 0.0
        color = pd.get("zone_color", "GREEN")
        is_trapped = bool(pd.get("trapped", False))

        summary_stats["total_population"] += pop
        summary_stats["total_demand"] += dem
        summary_stats["total_assignable_capacity"] += assignable
        summary_stats["total_unserved"] += unserved
        summary_stats["total_shelters"] += pd.get("capacity_summary", {}).get("shelters_total", 0)
        summary_stats["total_safe_shelters"] += pd.get("capacity_summary", {}).get("shelters_safe", 0)

        if is_trapped:
            summary_stats["trapped_zones_count"] += 1

        if color == "RED":
            summary_stats["red_zones_count"] += 1
        elif color == "YELLOW":
            summary_stats["yellow_zones_count"] += 1
        else:
            summary_stats["green_zones_count"] += 1
...
@app.get("/api/authority/scenario/{zone_id}")
def get_authority_scenario(zone_id: str, scenario_type: str = "live_forecast"):
    """Dynamic scenario timeline projection (t0 -> +1h -> +3h -> +6h) (FINAL doc §10).
    Projects hazard evolution using real Open-Meteo hourly weather forecast by default.
    """
    try:
        classification, _ = _score_zone(zone_id)
        worst_score = max(classification.scores.values()) if classification.scores else 0.0
    except Exception:
        worst_score = 0.50

    pop_rec = population_mod.get_population(zone_id)
    pop = pop_rec.population if pop_rec else None

    res = run_zone_scenario(
        zone_id=zone_id,
        base_hazard_score=worst_score,
        population=pop,
...
@app.get("/authority")
def serve_authority():
    html_path = _FRONTEND_DIR / "authority.html"
    if html_path.exists():
        return FileResponse(html_path)
    raise HTTPException(status_code=404, detail="authority.html not found")
